...
if __name__ == "__main__":
    ...
    rospy.init_node('navigation_goal_sender', anonymous=True)

    # Create an action client for move_base
    nav_as = actionlib.SimpleActionClient('/move_base', MoveBaseAction)

    # Wait for the action server to be available
    rospy.loginfo("Waiting for the move_base action server to be available...")
    if not nav_as.wait_for_server(rospy.Duration(5)):
        rospy.logerr("move_base action server is not available")
    else:
        rospy.loginfo("move_base action server is available")

        # Define the goal
        GOAL_POSITION = [2.0, 3.0]  # Example goal position
        ...
        rospy.loginfo("Sending goal: x: %s, y: %s, orientation: %s",
                      mb_goal.target_pose.pose.position.x,
                      mb_goal.target_pose.pose.position.y,
                      mb_goal.target_pose.pose.orientation)
    ...
    # Keep the node alive to ensure callbacks are processed
    rospy.spin()
    ...
    while compute_distance(goal_coor, curr_coor) > 1 and not collided and curr_time - start_time < 10000:
        curr_time = rospy.get_time()
        pos = gazebo_sim.get_model_state().pose.position
        curr_coor = (pos.x, pos.y)
    ...
    if collided:
        status = "collided"
    elif curr_time - start_time >= 10000:
        status = "timeout"
    else:
        status = "succeeded"
        success = True
