...
class CustomEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(4,), dtype=np.float32)  # 2 for current velocity + 2 for TEB velocity
        ...
        input_dim = 4  # 2 for current velocity + 2 for TEB velocity
    ...
    def teb_cmd_vel_callback(self, msg):
        # Ensure valid velocity data is received
    ...
    def step(self, action):
        # Collect the TEB command velocity
        teb_velocity_before_stop = self.teb_velocity.copy()
        ...
        # Calculate correction for the TEB command based on the current state
        state = np.concatenate((self.current_velocity, teb_velocity_before_stop))
        ...
        # Apply the correction to the TEB command
        corrected_action = teb_velocity_before_stop + correction

        # Cap the corrected action to avoid excessive values
        corrected_action = np.clip(corrected_action, self.action_space.low, self.action_space.high)

        # Publish the corrected action to the /cmd_vel topic
        twist = Twist()
        twist.linear.x = corrected_action[0]
        twist.angular.z = corrected_action[1]
        self.cmd_vel_pub.publish(twist)

        # Wait for the robot to execute the corrected command
        rospy.sleep(0.1)

        # Collect the actual velocity after executing the corrected command
        actual_velocity = self.current_velocity.copy()

        # Construct the observation with the new actual velocities
        observation = np.concatenate((actual_velocity, teb_velocity_before_stop))
        reward = self.compute_reward(action, correction)
        done = self.is_done()
        info = {}

        # Enhanced Logging for debugging
        print(f"State shape: {state.shape}, Next state shape: {observation.shape}")
        print(
            f"Action: {action}, Correction: {correction}, Corrected Action: {corrected_action}, Reward: {reward}, Done: {done}")
        print(
            f"Expected action: {teb_velocity_before_stop}, Actual action: {actual_velocity}, Corrected actual action: {corrected_action}")
        print(f"TEB Velocity: Linear: {teb_velocity_before_stop[0]}, Angular: {teb_velocity_before_stop[1]}")
        print(f"Current Position: {self.current_position}, Goal Position: {self.goal_position}")
        print(
            f"Relative Position to Goal: x={self.goal_position[0] - self.current_position[0]}, y={self.goal_position[1] - self.current_position[1]}")
        print(f"Processing time: {processing_time:.4f} seconds")
    ...
    def reset(self, seed=None, options=None):
        ...
        if not self.collision_detected and time_since_last_reset < 10.0:
            ...
            return np.concatenate((self.current_velocity, self.teb_velocity)), {}
        ...
        self.last_time = rospy.Time.now().to_sec()
        observation = np.concatenate((self.current_velocity, self.teb_velocity))
    ...
    def train_network(self, state, correction):
        # Stop the robot before training
        stop_twist = Twist()
        stop_twist.linear.x = 0
        stop_twist.angular.z = 0
        self.cmd_vel_pub.publish(stop_twist)

        # Start timing the training duration
        start_time = time.time()
